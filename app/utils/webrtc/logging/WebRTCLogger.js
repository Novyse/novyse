import { LOG_LEVELS, LOG_LEVEL_NAMES, LOG_COLORS, LOG_EMOJIS } from './LogLevels.js';
...
class WebRTCLogger {
  ...
  setLogLevel(level) {
    if (LOG_LEVELS && Object.values(LOG_LEVELS).includes(level)) {
      ...
      this.info('Logger', `Log level impostato a: ${LOG_LEVEL_NAMES?.[level] || level} (${level})`);
    } else {
  ...
  _log(level, category, message, ...data) {
    ...
    if (level < this.currentLogLevel) return;    const timestamp = new Date().toISOString();
  ...
  critical(category, message, ...data) {
    ...
    const formattedMessage = typeof message === 'object' ? safeStringify(message) : message;
    const formattedData = data.map(item => typeof item === 'object' ? safeStringify(item) : item);
    
    this._log(LOG_LEVELS?.CRITICAL || 10, category, formattedMessage, ...formattedData);
  }
  ...
  error(category, message, ...data) {
    ...
    this._log(LOG_LEVELS?.ERROR || 8, category, formattedMessage, ...formattedData);
  }
  ...
  warning(category, message, ...data) {
    ...
    this._log(LOG_LEVELS?.WARNING || 6, category, formattedMessage, ...formattedData);
  }
  ...
  info(category, message, ...data) {
    ...
    this._log(LOG_LEVELS?.INFO || 4, category, formattedMessage, ...formattedData);
  }
  ...
  debug(category, message, ...data) {
    ...
    this._log(LOG_LEVELS?.DEBUG || 2, category, formattedMessage, ...formattedData);
  }
  ...
  verbose(category, message, ...data) {
    ...
    this._log(LOG_LEVELS?.VERBOSE || 1, category, formattedMessage, ...formattedData);
  }
   ...
   */  printLogReport() {
    ...
    console.log(`🎛️  Livello corrente: ${LOG_LEVEL_NAMES?.[this.currentLogLevel] || this.currentLogLevel} (${this.currentLogLevel})`);
    console.log(`🔧 Logging abilitato: ${this.enabled ? '✅' : '❌'}`);
    console.log(`💾 Buffer abilitato: ${this.bufferEnabled ? '✅' : '❌'}`);
    console.log(`📝 Log in buffer: ${this.logBuffer.length}/${this.maxBufferSize}`);
    ...
    if (this.logBuffer.length > 0) {
      ...
      const categories = [...new Set(this.logBuffer.map(log => log.category))];

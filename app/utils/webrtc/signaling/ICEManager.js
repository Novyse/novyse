...
export class ICEManager {
    ...
    try {
      if (message.candidate) {
        ...
        if (!pc.remoteDescription) {
          this.logger.info(`Remote description non ancora impostata per ${participantId}, accodamento candidato`, {
            component: 'ICEManager',
            participantId
          });
  ...
  setupICEEventHandlers(pc, participantId) {
    this.logger.debug(`Configurazione gestori eventi ICE per ${participantId}`, {
      component: 'ICEManager',
      participantId
    });
  ...
  async processQueuedICECandidates(participantId) {
    ...
    const pc = this.globalState.getPeerConnection(participantId);
    if (!pc) {
      this.logger.error(`PeerConnection non trovata per processare candidati in coda: ${participantId}`, {
        component: 'ICEManager',
        participantId
      });
      return;
  ...
  async handleRemoteCandidate(participantId, candidateData) {
    ...
    try {
      if (candidateData) {
        ...
        if (!pc.remoteDescription) {
          this.logger.info(`Remote description not set for ${participantId}, queuing candidate`, {
            component: 'ICEManager',
            participantId
          });
          this._queueICECandidate(participantId, candidate);
      ...
      this.logger.error(`Error handling remote ICE candidate for ${participantId}`, {
        component: 'ICEManager',
        participantId,
        error: error.message
      });
  ...
  async getICEStatistics(participantId) {
    ...
    try {
      ...
      this.logger.error(`Errore ottenendo statistiche ICE per ${participantId}`, {
        component: 'ICEManager',
        participantId,
        error: error.message
      });
  ...
  _handleICECandidateGenerated(event, participantId) {
    if (event.candidate) {
      ...
      const webSocketMethods = require('../../webSocketMethods.js').default;      webSocketMethods.IceCandidate({
      ...
      this.logger.info(`Raccolta candidati ICE completata per ${participantId}`, {
        component: 'ICEManager',
        participantId
      });
    }
  ...
  _handleICEConnectionStateChange(pc, participantId) {
    ...
    const callback = this.globalState.getCallback('onPeerConnectionStateChange');
    if (callback) {
    ...
    switch (state) {
      case 'connected':
      case 'completed':
        this.logger.info(`Connessione ICE stabilita con successo per ${participantId}`, {
          component: 'ICEManager',
          participantId,
          iceConnectionState: state
        });
      ...
      case 'disconnected':
        this.logger.warning(`Connessione ICE disconnessa per ${participantId}`, {
          component: 'ICEManager',
          participantId,
          iceConnectionState: state
        });
  ...
  async _addICECandidateWithRetry(participantId, candidate) {
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        this.logger.debug(`Candidato ICE aggiunto con successo per ${participantId}`, {
          component: 'ICEManager',
          participantId,
        ...
        this.logger.warning(`Tentativo ${retryCount}/${maxRetries} fallito per candidato ICE di ${participantId}`, {
          component: 'ICEManager',
          participantId,
          attempt: retryCount,
          maxRetries,
        ...
        if (retryCount < maxRetries) {
          ...
          this.logger.error(`Tutti i tentativi falliti per candidato ICE di ${participantId}`, {
            component: 'ICEManager',
            participantId,
  ...
  _queueICECandidate(participantId, candidate) {
    ...
    this.logger.debug(`Candidato ICE accodato per ${participantId}`, {
      component: 'ICEManager',
      participantId,
      queueLength: this.globalState.getQueuedICECandidates(participantId).length
    });
  }

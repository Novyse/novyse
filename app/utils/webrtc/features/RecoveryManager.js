...
export class RecoveryManager {
    constructor(globalState, logger) {
        ...
        this.MAX_RECONNECTION_ATTEMPTS = WEBRTC_CONSTANTS.MAX_RECONNECTION_ATTEMPTS || 3;
        this.RECONNECTION_BASE_DELAY = WEBRTC_CONSTANTS.RECONNECTION_BASE_DELAY || 2000;
        this.CONNECTION_TIMEOUT = WEBRTC_CONSTANTS.CONNECTION_TIMEOUT || 30000;
        this.STABILIZATION_TIMEOUT = WEBRTC_CONSTANTS.STABILIZATION_TIMEOUT || 15000;
    ...
    async attemptConnectionRecovery(participantId) {
        if (!participantId) {
            this.logger.warn('RecoveryManager', 'Cannot recover empty participant ID');
            return false;
        }

        // Check if recovery is already in progress
        if (this.recoveryInProgress[participantId]) {
            this.logger.info('RecoveryManager', 
        ...
        if (currentAttempts >= this.MAX_RECONNECTION_ATTEMPTS) {
            this.logger.error('RecoveryManager', 
        ...
        this.logger.warn('RecoveryManager', 
    ...
    resetReconnectionAttempts(participantId) {
        if (this.reconnectionAttempts[participantId]) {
            ...
            this.logger.debug('RecoveryManager', 
    ...
    getRecoveryStatistics(participantId) {
        return {
            ...
            canAttemptRecovery: (this.reconnectionAttempts[participantId] || 0) < this.MAX_RECONNECTION_ATTEMPTS
        };
    }
    ...
    getAllRecoveryStatistics() {
        ...
        [...Object.keys(this.reconnectionAttempts), ...Object.keys(this.recoveryInProgress)]
    ...
    async _performConnectionRecovery(participantId) {
        ...
        const delay = this.RECONNECTION_BASE_DELAY * Math.pow(2, currentAttempt - 1);
        if (delay > 0) {
            this.logger.debug('RecoveryManager', 

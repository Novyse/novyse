...
export class HealthChecker {
    ...
    isConnectionHealthy(participantId) {
        ...
        const isConnectionHealthy = connectionState === 'connected' || 
                                                             connectionState === 'connecting';
        
        const isIceHealthy = iceConnectionState === 'connected' || 
    ...
    async _performHealthCheck(participantId) {
        ...
        try {
            const pc = this.globalState.getPeerConnection(participantId);
            if (!pc) {
                this._handleHealthCheckFailure(participantId, 'No peer connection found');
                return null;
            }
            ...
            if (!isHealthy) {
                ...
                return { participantId, healthy: false, reason: 'connection_unhealthy' };
            }
            ...
            this._handleHealthCheckFailure(participantId, `Health check error: ${error.message}`);
            return { participantId, healthy: false, reason: 'check_error', error: error.message };
        }
    }
    ...
    _handleHealthCheckFailure(participantId, reason) {
        ...
        this.consecutiveFailures[participantId] = (this.consecutiveFailures[participantId] || 0) + 1;

        this.logger.info(`Health check failed for ${participantId}: ${reason} (consecutive failures: ${this.consecutiveFailures[participantId]})`);
    ...
    async _triggerRecovery(participantId) {
        ...
        this.logger.info(`Triggering recovery for ${participantId} after ${this.consecutiveFailures[participantId]} consecutive failures`);
        ...
        try {
            ...
            this.logger.info(`Recovery attempt failed for ${participantId}: ${error.message}`);
        }
    }

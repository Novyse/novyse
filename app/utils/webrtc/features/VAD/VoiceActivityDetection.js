...
class VoiceActivityDetection {
    constructor(globalState, loggerInstance) {
        ...
        this.logger.debug('VoiceActivityDetection', 'VoiceActivityDetection initialized');
    }
    ...
    initialize(globalState) {
        ...
        this.logger.info('VoiceActivityDetection', 'VAD manager initialized successfully');
    ...
    async startVAD() {
        ...
        if (!this.globalState.localStream) {
            this.logger.warn('VoiceActivityDetection', 'No local stream available for VAD');
            return false;
        }
        ...
        try {
            ...
            this.logger.error('VoiceActivityDetection', 'Failed to start VAD:', error);
            return false;
        }
    }
    ...
    setSpeakingThreshold(threshold) {
        if (typeof threshold !== 'number' || threshold < 0 || threshold > 1) {
            this.logger.warn('VoiceActivityDetection', 'Invalid threshold value:', threshold);
            return;
        }

        this.globalState.speakingThreshold = threshold;
        this.logger.debug('VoiceActivityDetection', `Speaking threshold set to ${threshold}`);
    }
    ...
    setSpeakingState(participantId, isSpeaking) {
        try {
            ...
            this.logger.verbose('VoiceActivityDetection', `Speaking state set for ${participantId}: ${isSpeaking}`);
        } catch (error) {
            this.logger.error('VoiceActivityDetection', 'Error setting speaking state:', error);
        }
    }
    ...
    cleanup() {
        ...
        try {
            ...
            this.logger.error('VoiceActivityDetection', 'Error during VAD cleanup:', error);
        }
    }
}

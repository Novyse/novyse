...
export class MediaUtils {
    ...
    getOptimalVideoConstraints(options = {}) {
        ...
        this.logger?.log(LogLevels.DEBUG, 'Generated optimal video constraints', baseConstraints);
    ...
    getOptimalAudioConstraints(options = {}) {
        ...
        this.logger?.log(LogLevels.DEBUG, 'Generated optimal audio constraints', constraints);
        return constraints;
    }
    ...
    async supportsConstraints(constraints) {
        try {
            ...
            this.logger?.log(LogLevels.DEBUG, 'Constraints support check', { constraints, supported });
            return supported;
        } catch (error) {
            this.logger?.log(LogLevels.WARNING, 'Constraints not supported', { constraints, error: error.message });
            return false;
        }
    }
    ...
    getFallbackConstraints(error, originalConstraints) {
        ...
        if (originalConstraints.video && error.name === 'NotFoundError') {
            ...
            this.logger?.log(LogLevels.INFO, 'Falling back to audio only due to video error');
        }
        // If permissions denied, try most basic constraints
        else if (error.name === 'NotAllowedError') {
            fallback.audio = true;
            this.logger?.log(LogLevels.INFO, 'Falling back to basic audio due to permissions');
        }
        // For other errors, try very basic constraints
        else {
            fallback.audio = { echoCancellation: false, noiseSuppression: false };
            this.logger?.log(LogLevels.INFO, 'Falling back to minimal audio constraints');
        }
    ...
    getAudioQualityRating(settings) {
        ...
        if (sampleRate >= 44100 && channelCount >= 2 && echoCancellation && noiseSuppression) {
            return 'ultra';
        }

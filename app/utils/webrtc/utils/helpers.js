...
export function isConnectionFailed(peerConnection) {
  ...
  return (
    iceState === "failed" || connState === "failed" || connState === "closed"
  );
}
...
export async function withRetry(operation, maxAttempts = 3, baseDelay = 1000) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      logger.warning(
        "Helpers",
        `Tentativo ${attempt}/${maxAttempts} fallito, retry in ${delay}ms:`,
        error.message
      );
      await wait(delay);
    }
...
export function cleanupMediaStream(stream, logContext = "unknown") {
  ...
  try {
    ...
    logger.debug(
      "Helpers",
      `Pulizia ${tracks.length} tracce per ${logContext}`
    );
    ...
    tracks.forEach((track) => {
      try {
        ...
        logger.warning(
          "Helpers",
          `Errore fermando traccia per ${logContext}:`,
          error
        );
      }
    });

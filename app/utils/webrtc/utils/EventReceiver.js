...
class WebRTCEventReceiver {
    constructor(logger,globalState) {
        ...
        this.boundHandlers = {
            ...
            remoteUserStartedSpeaking: this.handleRemoteUserStartedSpeaking.bind(this),
            remoteUserStoppedSpeaking: this.handleRemoteUserStoppedSpeaking.bind(this),
    ...
    setupEventListeners() {
        ...
        eventEmitter.on(`user_started_speaking`, this.boundHandlers.userStartedSpeaking);
        eventEmitter.on(`user_stopped_speaking`, this.boundHandlers.userStoppedSpeaking);
        eventEmitter.on(`remote_user_started_speaking`, this.boundHandlers.remoteUserStartedSpeaking);
        eventEmitter.on(`remote_user_stopped_speaking`, this.boundHandlers.remoteUserStoppedSpeaking);

        // Screen Sharing Events
        eventEmitter.on(`screen_share_started`, this.boundHandlers.screenShareStarted);
        eventEmitter.on(`screen_share_stopped`, this.boundHandlers.screenShareStopped);
    ...
    async handleUserStoppedSpeaking() {
        if (this.voiceActivityDetection) {
            this.voiceActivityDetection.setSpeakingState(this.globalState.myId, false);
        }
        await WebSocketMethods.sendSpeakingStatus(
            this.globalState.getChatId(),
            this.globalState.myId,
    ...
    async handleMemberJoined(data) {
        // Check if the member is not the current user and if my id is defined
                if (!this.initialized || !this.globalState || this.globalState.myId === undefined) {
            this.logger?.info(
                `[EventReceiver] handleMemberJoined: Instance not ready, globalState is null, or myId is undefined. Initialized: ${this.initialized}, GlobalState: ${!!this.globalState}, MyId: ${this.globalState?.myId}`
            );
            return;
        }
    ...
    removeEventListeners() {
        // Voice Activity Detection Events
        eventEmitter.off(`user_started_speaking`, this.boundHandlers.userStartedSpeaking);
        eventEmitter.off(`user_stopped_speaking`, this.boundHandlers.userStoppedSpeaking);
        eventEmitter.off(`remote_user_started_speaking`, this.boundHandlers.remoteUserStartedSpeaking);
        eventEmitter.off(`remote_user_stopped_speaking`, this.boundHandlers.remoteUserStoppedSpeaking);

        // Screen Sharing Events
        eventEmitter.off(`screen_share_started`, this.boundHandlers.screenShareStarted);
        eventEmitter.off(`screen_share_stopped`, this.boundHandlers.screenShareStopped);

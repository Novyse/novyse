...
export class ConnectionTracker {
  ...
  reportConnectionEvent(participantId, event, data = null) {
    ...
    this.logger.debug('ConnectionTracker', 
      `[${timestamp}] ${participantId}: ${event}`, data || "");
  ...
  logConnectionDebugInfo(participantId, context) {
    ...
    const debugInfo = {
      ...
      queuedCandidates: this.globalState.iceCandidateQueues[participantId]?.length || 0,
    };
    
    this.logger.debug('ConnectionTracker', `${participantId} debug:`, debugInfo);
  }
  ...
  printConnectionReport() {
    ...
    this.logger.info('ConnectionTracker', 
      `🎤 Local Stream: ${stats.hasLocalStream ? `✅ (${stats.localStreamTracks} tracks)` : "❌"}`);
    this.logger.info('ConnectionTracker', `🔗 Total Connections: ${stats.totalConnections}`);
    this.logger.info('ConnectionTracker', `⚙️ Health Check Interval: ${stats.healthCheckInterval}ms`);
    this.logger.info('ConnectionTracker', 
    ...
    if (stats.totalConnections === 0) {
      ...
      Object.entries(stats.connections).forEach(([id, conn]) => {
        ...
        this.logger.info('ConnectionTracker', 
          `   ⏰ Age: ${conn.connectionAge}s | Last Good: ${conn.lastGoodConnection ? conn.lastGoodConnection + "s ago" : "Never"}`);
        this.logger.info('ConnectionTracker', 
          `   📺 Remote Stream: ${conn.hasRemoteStream ? `✅ (${conn.remoteStreamTracks} tracks)` : "❌"}`);
        this.logger.info('ConnectionTracker', 
          `   📋 Queued Candidates: ${conn.queuedCandidates} | Negotiating: ${conn.negotiationInProgress ? "✅" : "❌"}`);
      });
    }
  ...
  clearTracking(participantId) {
    this.logger.debug('ConnectionTracker', `Clearing tracking for ${participantId}`);
  ...
  updateLastKnownGoodState(participantId) {
    ...
    this.logger.verbose('ConnectionTracker', `Updated last good state for ${participantId}`);
  }
  ...
  _getStatsForParticipant(participantId) {
    ...
    return {
      ...
      lastGoodConnection: this.globalState.lastKnownGoodStates[participantId]
        ...
        : null,
      queuedCandidates: this.globalState.iceCandidateQueues[participantId]?.length || 0,
      negotiationInProgress: this.globalState.negotiationInProgress[participantId] || false,
      hasRemoteStream: !!this.globalState.remoteStreams[participantId],
      remoteStreamTracks: this.globalState.remoteStreams[participantId]?.getTracks()?.length || 0,
    };
  }

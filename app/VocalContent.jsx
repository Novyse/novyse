  ...
  const getExistingScreenShareStream = (streamId, participantId = null) => {
    try {
      ...
      if (stream) {
        ...
        console.log(
          `[VocalContent] No existing stream found for ${streamId} (participant: ${participantId})`
        );
        ...
        const remoteScreenStreams = webrtcManager.getRemoteScreenStreams();
        console.log(
          `[VocalContent] All available local screen streams:`,
          Object.keys(allScreenStreams)
        );
        console.log(
          `[VocalContent] All available remote screen streams:`,
          remoteScreenStreams
        );
      }
      return stream;
      ...
      console.log(
        `[VocalContent] Error getting existing stream for ${streamId}:`,
        error.message
      );
      return null;
    }
        ...
        if (
          ...
          profile.active_screen_share.forEach((streamId) => {
            ...
            if (existingStream && profile.from === get.myPartecipantId()) {
              console.log(
                `[VocalContent] Restoring local screen share to WebRTC manager: ${streamId}`
              );
              const webrtcManager = require("./utils/webrtc/index.js").default;
    ...
    setProfilesInCommsChat((currentProfiles) => {
      ...
      setActiveStreams((prevStreams) => {
        ...
        if (hasExistingActiveStream) {
          console.log(
            `[VocalContent] Stream ${streamId} already exists in activeStreams, updating userData`
          );
          ...
          console.log(
            `[VocalContent] Creating new activeStreams entry for ${streamId}`
          );
      ...
      console.log(
        ...
        } screen share rectangle with streamId: ${streamId}`
      );
